...
```{r} 
#testing 
test_model <- stm_eval_model[1, ]

my_stm_eval(list(test_model$documents, test_model$vocab, test_model$n_topic))

my_stm_eval(documents = test_model$documents, vocab = test_model$vocab, n_topic = test_model$n_topic)

test <- list(test_model$documents, test_model$vocab, test_model$n_topic)
test %>% my_stm_eval

test <- list(stm_eval_model$documents, stm_eval_model$vocab, stm_eval_model$n_topic)

test %>% my_stm_eval


test <- stm_eval_model %>% mutate(test = list(documents, vocab, n_topic) %>% pmap(list))

stm_eval_model %>% mutate(test = test %>% map(my_stm_eval))
test1<- test$test[[1]]
