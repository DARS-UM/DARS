...
#
##LDA_models
#load("Output/LDA_overview.RDATA") #replace with underneath
...
##Inspect topics
```{r inspect topics}
topics <- clean_models$beta_distribution$k10 %>%
  group_by(topic) %>%
  top_n(10, beta)
...
student <- list()
student$kw_policy <- c("international", "economic", "conflict", "develop", "policy", "war", "philosophy")
student$kw_climate <- c("climate", "sustainability", "change", "development", "develop","sustain", "sustainable")
```

##getting top courses per student
```{r topic score student}
student$topic_score <- clean_models$beta_distribution$k35 %>%
...
student$course_score <- clean_models$gamma_distribution$k35     %>%
...
##Selectig keywords
```{r}
top_words <- beta_description$k35 %>%
  group_by(topic) %>%
  top_n(3, beta) %>%
  pull(term)%>%
  unique
...
kw_stopwords =c(
  "student", "skill", "academic", "bate", "participant", "gain", "primarily", "main", "2016", "action", "level", "issue",
  "concept", "close", "day", "edition", "primary", "project", "method", "apply", "busy", "address", "role", "post", "oxford",
  "cuss", "lecture", "pbl",
    #unsure:
  "theory", "search", "perspective"
                )
kw <- setdiff(top_words, kw_stopwords)

save(kw, file="App/Recommender System/LDA_keyword_DELETE_.RDATA")
```
