...
my_control <- list(
  ...
  nstart = 10,
  seed   = 1 : 10, #must have length nstart
  best   = TRUE,
...
#Training
topic_numbers <-c(50, 55, 60, 65, 70) #seq(from = 5, to = 10, by = 2) #5 to 150 by 50
...
#Save
save(tb_topic_model, file = paste("./Output/", which_texts, "tmp_topic_model_tibble.RDATA", sep = "_"))
...
app_topic_model <- tb_topic_model %>% select(`Topic ID`, n_topic, Origin, Gamma, Beta, kw) %>%
...
```{r app files, eval = T}
save(app_topic_model, app_model, file = "App/Recommender System/data_topic_models.RDATA")
```
