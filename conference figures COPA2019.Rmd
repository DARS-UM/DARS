...
d_width <- conformal_results %>%
  group_by(course, significance) %>%
  summarise(q_50 = quantile(width, 0.5), # median
            q_10 = quantile(width, 0.1),
            q_90 = quantile(width, 0.9))
```

```{r}
d_width %>%
  ggplot() +
  geom_errorbar(aes(significance, ymax = q_90, ymin = q_10), size = .5) +
  geom_hline(yintercept = 1, linetype = "dashed") +
  geom_line(aes(significance, q_50, group = 1)) +
  geom_point(aes(significance, q_50)) +
  facet_wrap( ~ course) +
  labs(y = "Width",
...
```{r}
d_error %>%
  ggplot(aes(x = significance, y = error)) +
...
```{r}
conformal_results %>%
  filter(significance %in% c(0.05, 0.1, 0.2)) %>%
  group_by(course, significance) %>%
  ...
  unite(id, measure, significance, sep = "-") %>%
