...
##key word Contributions to Topic
 contribution_to_topics #<- 
beta_description$k35                       %>%
...
###Selecting key words
```{r}
test <- beta_description$k35 %>%
  group_by(topic) %>%
  top_n(3, beta) %>%
  group_by(topic) %>%
  summarise(key_words= list(Reduce(f= c, x = term)))

kw <- unique(Reduce(x = test$key_words, f= c))
