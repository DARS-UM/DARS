...
covariates <- tribble(
  ~Origin,    ~Data,
  "catalogues", d_text$catalogues %>% distinct(`Course ID`, department),
  "manuals",    d_text$manuals %>% mutate(department = "UCM") %>% distinct(`Course ID`, department)
)
```
###STM Corpus
...
my_stm <- function(n_topics, corpus, data){
  ...
  STM_model <- stm(corpus$documents, corpus$vocab, data = data, prevalence = ~department, ### ~department
                   K = n_topics, init.type = "Spectral")
}
```
...
#Extract covariates
extract_covariates <- function(origin) covariates %>% filter(Origin == origin) %>% pull(Data)
...
preparation_for_model <- tibble(
  ...
  #left_join(covariates, by = "Origin") %>%
  ...
  mutate(
    ...
    #Covariates
    Covariates        = pmap(.l = list(Origin),  .f = extract_covariates), # testing_model$Covariates[4][[1]][[1]]
    
    #clean_covariates
    clean_covariates = map(Covariates, .f = function(Covariates) Covariates[[1]]))


#iteration
this_model <- preparation_for_model %>% filter(n_topic==5, Origin=="manuals")

this_n_topic <- this_model$n_topic[[1]]
this_corpus <- this_model$Corpus[[1]]
this_clean_covariates <- this_model$clean_covariates[[1]]

this_model <- my_stm(this_n_topic, this_corpus, this_clean_covariates)
  
    #Fit Model  n_topics, corpus, data
    Model  = pmap(.l = list(n_topics = n_topic, corpus = Corpus, data = clean_covariates), 
                  .f = my_stm))
...
select_topic_model <- function(origin, n_topic){
  model <- stm_model %>% 
    filter(Origin == origin & n_topic == n_topic) %>%
    pull(Model)
...
plot.STM(test, "perspectives", topics = c(1,5))
```
#New Documents
##Align vocabulary
```{r align vocab}
...
newdocs <- alignCorpus(new=out, old.vocab = model$vocab)
fasos_theta <- fitNewDocuments(model = model, documents=newdocs$documents) #returns a course by topic matrix
...
#Experiment
```{r experoment}
estimateEffect(formula = 1:5 ~department, meta = test$meta)
