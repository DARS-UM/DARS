...
###mystm_manuals
```{r my_STM, eval= TRUE}
my_stm_manuals <- function(n_topics){

  STM_model <- stm(manuals_corpus$documents, manuals_corpus$vocab, data = manuals_covariates, prevalence = ~department, K = n_topics, init.type = "Spectral")
}
```
...
```{r models_manuals in tibble, error=TRUE}
topic_numbers <- seq(from = 5, to = 10, by = 5) # seq(from = 5, to = 20, by = 5) #seq(from = 5, to = 150, by = 5)

model_tmp_result <- vector("list", length(topic_numbers))

for(i in seq_along(topic_numbers)){
  k <- topic_numbers[[i]]
  model_tmp_result[[i]] <- stm(documents = manuals_corpus$documents, 
                               vocab = manuals_corpus$vocab, 
                               K = k, 
                               #data = manuals_covariates, 
                               #prevalence = ~department, 
                               init.type = "Spectral"
                               )
}
stm_model_manuals <- many_models(
  K = 5:10,
  documents = manuals_corpus$documents,
  vocab= manuals_corpus$vocab,
  prevalence = ~ department, 
  data = manuals_covariates,
  runs = 5
