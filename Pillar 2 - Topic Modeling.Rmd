...
`d_course`, `d_transcript`, and a list `d_text` with the textual data for catalogues and manuals (for $catalogues, $manuals we have (`Course ID`, `year`, `word_original`, `word`) ).
...
topic_numbers <- seq(from = 4, to = 150, by = 2)

test_topic_model <- tibble( #CHANGE NAME TO: topic_model
  
  #repeat each topic number twice (once for catalogues and once for manual)
  n_topic = rep(topic_numbers, each = 2) # from = 5, to = 250, by = 5
  
  ) %>%
  mutate(Origin     = rep(c("catalogues", "manuals"), 74),         # should be 50
...
```{r save, eval = T}
save(test_topic_model, file = "Output/test_topic_model.RDATA" )
