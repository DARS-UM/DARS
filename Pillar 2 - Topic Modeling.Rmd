...
NOT NEEDED TO BE FINISHED
```{r manual evaluation, cache = TRUE, eval = TRUE}
set.seed(1234)

#number of topics for trials
...
#crossv_sets returns data.frame with 10-folds divided into train (DTM) and test sets(DTM)
...
cross_validate <- function(my_dtm){
  #Number of documents (D)
  D <- nrow(my_dtm)
  
  #Assign documents to a fold
  folding <- sample(rep(1:10, ceiling(D))[seq_len(D)])
  
  for(k in n_topics) {
    for(chain in 1:10){
      #Sets
      train_set <- my_dtm[folding != chain, ]
      test_set  <- my_dtm[folding == chain, ]
      
      #LDA
      training      <- LDA(train_set, k = k, control = my_control, method = "Gibbs")
      
      #best_training <- training@fitted[[which.max(logLik(training))]]
      return(perplexity(training, newdata = test_set))
      #testing  <- my_LDA(test_set, model = best_training, control = list())
    }
  }
}
...
D_overview <- nrow(d_cast$overview)
D_manual <- nrow(d_cast$manual)

###Assigning random numbers (1:10) to each row of our DTM
folding_overview <- sample(rep(seq_len(10), ceiling(D_overview))[seq_len(D_overview)])
folding_manual   <- sample(rep(seq_len(10), ceiling(D_manual))[seq_len(D_manual)])

for(k in n_topics){
  for(chain in 1:10){
    ###Divide into training and testing sets
      #overview
    training_set_overview <- d_cast$overview[folding!=chain,]
    test_set_overview     <- d_cast$overview[folding==chain,]
      #manual
    training_set_manual   <- d_cast$manual[folding!=chain,]
    test_set_manual       <- d_cast$manual[folding==chain,]
