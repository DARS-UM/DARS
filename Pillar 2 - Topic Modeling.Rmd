...
visualize_LDA <- function(results, id_plot = "test", facet = F, topic_names = NULL){
  ...
  # Beta distribution
  tidy(LDA_description_10, matrix = "beta") %>%
  ...
  # Gamma Distribution
  data_gamma <- tidy(LDA_description_10, matrix = "gamma") %>%
    mutate(topic = paste("Topic", topic)) %>%
    rename(Code = document) %>%
    left_join(select(d_course, Code, Cluster, Concentration, `Concentration (additional)`), by = "Code")
  
  if(ensym(facet) == sym("Course")) data_gamma <- data_gamma %>%
                                      rename(facet = Code)
  
  if(ensym(facet) == sym("Cluster")) data_gamma <- data_gamma %>%
                                       filter(is.na(Cluster)) %>%
                                       rename(facet = Cluster)
                                       
  if(ensym(facet) == sym("Concentration")) data_gamma <- data_gamma %>% 
                                             filter(!is.na(Concentration)) %>%
                                             gather(X, Concentration, Concentration, `Concentration (additional)`, na.rm = TRUE) %>%
                                             rename(facet = Concentration)
  
  
  # Gamma Distribution: plot 1 docu-topic
  data_gamma %>%
