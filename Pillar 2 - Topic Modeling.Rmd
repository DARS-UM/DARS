...
```
#DELETE LATERRRRRRRRRRRRRRRRRRRRRRRRRRRRRR
Include d_course
```{r d_course}
load("Output/data_general.RDATA")

rm(d_AoD, d_assessment, d_ILO, list_AoD_assessment)
```
# TF-IDF
...
#extract dtm
extract_dtm <- function(wt) d_cast[[wt]]
...
plot_evaluation <- function(){
  ...
  test_topic_model %>%
    ...
    geom_point() +
    facet_wrap(`Eval Type` ~ Origin, scales = "free") +
    theme_minimal()
...
```
##Saving filed for different sessions
For internal use:
```{r}
# save(topic_model, file = "Output/topic_model.RDATA" )
```
...
```
#Distributions
#NOTE: We need to select how many topics to use
