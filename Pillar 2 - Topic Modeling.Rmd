...
We train the LDA for different numbers of topics (5 to 250 by 5), we save the resulting 50 models under `topic_model`
###Functions for models (get gamma, get beta, extract_dtm)
...
test_topic_model <- tibble( #change to topic_model
  #repeat each topic number twice
  ...
  mutate(Origin     = rep(c("overview", "manuals"), 3),         # should be 50
         ...
         #fit model
         Model  = pmap(.l = list(n_topic, DTM), .f = my_LDA, control = my_control_fast),
...
#View(test_topic_model)
test_topic_model
```
#DELETE LATERRRRRR- HELPER IMPORT
```{r}
...
#Distributions
#NOTE: select how many topics to use
...
extract_overview <- function() topic_model_gb %>% filter(n_topic == n_topics_overview)
extract_manual   <- function() topic_model_gb %>% filter(n_topic == n_topics_manual  )



df %>% mutate(var_new = list(var_old1, var_old2) %>% pmap(my_function))
...
data <- tribble(
  ...
  mutate(df = if_else(ID == "overview", extract_overview(), extract_manual))
...
# FUNCTION
#get_top_match <- function(<INPUT GAMMA OR BETA DISTRIB>){<FUNCTION GENERALIZATION>}
get_top_match <- function(d_distribution){
  print(!!sym(d_distribution))
}
Hi <- "hello"
get_top_match(Hi)
...
#Saving to models
test_topic_model <-  test_topic_model %>%
  mutate(top_terms_topic = map(Beta, .f = get_top_terms_topic))


test_topic_model <- test_topic_model %>%
  mutate(top_topic_course = map(Gamma, .f = get_top_topics_course))
...
go_down <- function(my_list, i){
  my_list[[i]]
}

get_key_words <-  function(d_top_words){
...
a <- test_topic_model[["top_terms_topic"]]



d_top_words <- a[1]

#mutate(kw = get_key_words(top_terms_topic)) 
d_top_words <- a$top_terms_topic
