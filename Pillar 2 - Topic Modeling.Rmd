...
#create folds
create_folds <- function(my_dtm){
  #find number of documents (D)
  D <- nrow(my_dtm)
...
topic_test %>% mutate(loglike_overview = map(.x = TM_overview, .f = my_loglike)) %>% select(n_topic, loglike_overview)


  for( i in 1:length(topic_model$TM_overview)){
    tmp <- 
  }
}
...
lL <- list()
for(i in 1:length(topic_model$n_topic)){
  
  tmp <- topic_model$TM_overview[i]
  lL[[paste("n", 2*i)]] <- tmp[[1]]@loglikelihood
  
}

test1[[1]]@loglikelihood

log_like <- logLik(test1[[1]])
