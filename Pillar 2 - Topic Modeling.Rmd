...
plot_tf_idf <- function(data, facet, top = 10, n_col = 5){
  
  facet_enquo <- enquo(facet)
  facet_quo <- quo(!!facet_enquo)

  facet_ensym <- ensym(facet)
  
  return(facet_ensym == sym("Cluster"))
  
}

plot_tf_idf(d_course, facet = Cluster)
  ...
  data <- if(facet = quo(Course)) filter(data, Code %in% sample(unique(.$Code), size = n_col^2, replace = F))
          else if(facet = quo(Cluster)) left_join(data, select(d_course, Code, Cluster), by = "Code")
