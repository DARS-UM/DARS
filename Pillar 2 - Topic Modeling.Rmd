...
topic_numbers <-seq(from = 5, to = 150, by = 5)
...
If we trained separately but with the same number of topics, and wish to merge and save them:
```{r merge if trained separately, eval = TRUE}
cata <- catalogues_topic_model$n_topic
man  <- manuals_topic_model$n_topic
...
##Best model
We extract the best model for the catalogues and use it to innitialise the the second run of LDA algorithm with more burns and iterations
```{r best model, eval = T}
catalogues_tm <- tb_topic_model %>% fil
