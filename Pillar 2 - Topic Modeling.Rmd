...
topic_model <- tibble(
  ...
  n_topic = seq(from = 5, to = 250, by = 5)
...
##Model Evaluation
###Perplexity
Perplexity is a measure of how well a probability model predicts a sample. (Thus may want to do cross validation.) 
```{r perplexity function, eval = TRUE}
...
#Log likelihood:
test_likelihood <- topic_model %>% mutate(logLik_overview = map(.x = TM_overview, .f = logLik),
...
#perplexity
plot(test_perplexity$n_topic, test_perplexity$perplex_overview)
plot(test_perplexity$n_topic, test_perplexity$perplex_manual)

#log likelihood
plot(test_likelihood$n_topic, test_likelihood$logLik_overview)
plot(test_likelihood$n_topic, test_likelihood$logLik_manual)
```
