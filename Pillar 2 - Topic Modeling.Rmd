...
plot_tf_idf <- function(data, facet = Course, top = 10, n_col = 5, plot_id = NULL){
  
  # Setup
  facet_enquo <- enquo(facet)
  facet_ensym <- ensym(facet)
  
  # Data
  
  data <- 
    if(facet_ensym == sym("Course")) data %>%
      filter(Code %in% sample(unique(.$Code), size = n_col^2, replace = F)) %>% # random selection of courses
      rename(facet = Code) %>%
      group_by(facet) %>%
      filter(n >= 2) %>%
      top_n(top, tf_idf) %>%
    ...
    else if(facet_ensym == sym("Cluster")) data %>%
      left_join(data, select(d_course, Code, Cluster), by = "Code") %>%
      rename(facet = !!facet_enquo) %>%
      group_by(facet) %>%
  ...
  g <- data %>%
    ...
    ggsave(paste(plot_id, facet_ensym, "_BP.jpeg", sep = ""), path = "Plots", width = 16, height = 8)
    print(g)
  ...
  ggsave(paste(plot_id, facet_ensym, "_WC.jpeg", sep = ""), path = "Plots", width = 16, height = 8)
  print(g)
...
## Course Level
```{r tf-idf per course, out.height=8, out.width=16}
set.seed(123)
plot_tf_idf(tf_idf_description, plot_id = "description_")
set.seed(123)
plot_tf_idf(tf_idf_overview, plot_id = "overview_")
set.seed(123)
plot_tf_idf(tf_idf_manual, plot_id = "manual_")
```
