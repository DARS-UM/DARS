...
COMMENTS: 
Q. Jerry -> For preprocessing (selecting vocabulary), how do we assign words that appear in only one document to a topic? Does it make sense to remove them?
...
# Setup
We load the environment `output/data_pillar_2.RDATA`. It contains the list `d_text`, and the datasets `d_course` and `d_transcript`.
...
To prepare the output of pillar two, we will need d_course. Thus we load the environment `output/data_pillar_1.RDATA`. It contains the datasets d_course and d_transcript.
```{r}
#
