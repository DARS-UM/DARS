...
# Setup
symbol_to_line <- c("\r\n", "\n") %>% paste(collapse = "|")
start_descrip  <- c("^Description", "^Course Description") %>% paste(collapse = "|")
end_descrip    <- c("^Recommended Literature", "^ ?Required Litera", "^ ?Literature$", "Literature \\(all") %>%  paste(collapse = "|")
...
is_end_descrip_bis <- function(x) x %>% str_detect(pattern = "^Instructional format$")
...
extract_description <- function(page){
  ...
  page %>%
    ...
    mutate(
      ...
      is_end_descrip   = line %>% is_end_descrip
    ) %>%
    
    mutate(
      is_end_descrip = if_else(any(is_end_descrip), is_end_descrip, line %>% is_end_descrip_bis)
    ) %>%
...
d_text$overview %>%
  mutate(
    description = overview %>% map(extract_description)
  )
```
