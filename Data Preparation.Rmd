...
```{r extract_description, eval = FALSE}
extract_description <- function(overview){
  ...
  overview %>%
    ...
    mutate(
      ...
      is_end_descrip   = line %>% is_end_descrip # Having several ends (e.g. "literature" and "instructional format") is ok because we only use the first (cumany)
      ) %>%
