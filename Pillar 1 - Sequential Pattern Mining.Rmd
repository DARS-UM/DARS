...
transactions <- tribble(
  ~ ID   ,  ~ data                    , ~ item,
  "taken", d_transactions$taken_PF_HL, quo(item),
  "PF"   , d_transactions$taken_PF_HL, quo(item_PF),
  "HL"   , d_transactions$taken_PF_HL, quo(item_HL),
  "TPF"  , d_transactions$T_PF_HL    , quo(item_TPF),
  "THL"  , d_transactions$T_PF_HL    , quo(item_TPF),
  "G"    , d_transactions$G          , quo(item_g)
  ) %>%
  
  mutate(transactions = list(data, item) %>% pmap(make_transaction))

inspect(head(transactions$transactions[[2]], 10))
```
...
sequences <- tribble(
  ...
  mutate(sequences = list(data, item) %>% pmap(make_sequence))

inspect(head(sequences$sequences[[2]], 10))
```
