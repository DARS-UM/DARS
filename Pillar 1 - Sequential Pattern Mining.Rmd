...
# TODO: compute lhs.support, for grade, not taken and  
```
...
compute_support <- function(AR, data_support, type_rule){
  ...
  AR %>%
    ...
    # include lhs.support
    mutate(
      lhs_course = str_remove_all(lhs, pattern = "_fail|_low")
      ) %>%
    left_join(
    ...
    # include rhs.support
    mutate(
      rhs_course = str_remove_all(rhs, pattern = "_fail|_low")
      ) %>%
    left_join(
    ...
    # order variable
    select(
      lhs, rhs, support, count, confidence, rhs.support, lift, lhs.support
    )
