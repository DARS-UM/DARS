...
clean_IR <- function(rules){
  rules %>%
    ...
    select(
      lhs, rhs,
      support, 
      #count, lhs.support, #figure out how to get these
      confidence, rhs.support,
      lift
    ) #%>%
    #arrange(
...
#Generating induced rules
IR    <- my_IR(data = sequences   )
IR_PF <- my_IR(data = sequences_PF)
IR_HL <- my_IR(data = sequences_HL)

#Saving induced rules:
save(
  IR = list(
    IR = IR,
    IR_PF = IR_PF,
