...
d_cumulative_transcript <-  d_transactions        %>%
  ...
  mutate(
    Cumulative_transcript=lag(Reduce(paste, item, accumulate=T)),
    Cumulative_transcript= str_split(Cumulative_transcript," ")
                                     )
```
...
my_apriori <- function(data, appearance = NULL){
  ...
  data %>%
    apriori(
      ...
      # impose restriction on what could be on the lhs and rhs of the rule
...
```{r}
compute_support <- function(AR, type_rule){
  
  type_rule_enquo <- enquo(type_rule)
  
  AR %>%
    left_join(
      select(d_support, item, !!type_rule_enquo),
      by = c("rhs" = "item")
      ) %>%
    mutate(
    ...
    select(
      lhs, rhs, support, count, confidence, rhs.support, lift
    )
...
```{r}
make_AR <- function(data, appearance, type_rule){
  
  data %>%
    my_apriori(appearance = appearance) %>%
    clean_AR() %>%
    compute_support(type_rule = type_rule)
...
AR_take <- make_AR(transactions)

# Generating asocciation rules
AR_taken <- my_apriori(transactions) %>%
  left_join(
    select(d_support_take, item, rate.take),
    by = c("rhs" = "item")
    ) %>%
  mutate(
    rhs.support = round(rate.take, 5)
    ) %>%
  select(
    - rate.take
    ) %>%
  ...
  mutate(
    lhs.support = round(rate.take, 5)
    ) %>%
  select(
    - rate.take
    ) %>%
  select(lhs, rhs, support, count, confidence, rhs.support, lift)
