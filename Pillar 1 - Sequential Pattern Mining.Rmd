...
edit_AR <- function(AR){
  ...
  AR %>%
    ...
    select(
      ...
      lift, 
      everything(),
      - matches("rate|course|grade|outcome|taken")
    ) %>%
    arrange(
      desc(count)
    )
...
# Remove objects
rm(
  my_apriori, clean_AR, compute_metrics, edit_AR, make_AR,
  AR, transaction
  )
```
...
my_SR<- function(data){
 data %>%
  cspade(
    parameter = list(
      #include as many item sequences as possible
    ...
    ruleInduction(
      #include as many rules as possible
      confidence = 0,
...
SR_taken   <- my_SR(data = sequences$taken )
```
...
SR_TPF <- d_cumulative_transcript                  %>%
  select(sequenceID, eventID, item, course_not)    %>%
  unnest(item, .drop = F)                          %>%
  unnest(course_not)                               %>%
  left_join(
    select(d_transactions, 
           sequenceID, eventID, item, PF), 
    by= c("sequenceID", "eventID", "item")
    )                                              %>%
  select(sequenceID,eventID, course_not, item, PF) %>%
  rename(
    lhs="course_not", 
    rhs.x = "item", 
    rhs.PF = "PF")                                 %>%
  ...
  mutate(
    ...
    confidence_bounded           = num_rule_PF/num_rule_x, #support_rhs_bounded / support_lhs, #not sure about this 
