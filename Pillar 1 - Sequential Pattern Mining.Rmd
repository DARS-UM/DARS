...
find_course <- function(data = d_transcript, code){
  c = code
  x <- data %>%
  ...
  mutate(Outcome = 
           case_when( Grade=="NG"~"Fail", #if we don't do this NG satisfies the condition for pass v
                      Grade > 5.5 ~"Pass",
                              T~"Fail"),
         Low_grade =
           case_when(Grade=="NG"~"Sad", #if we don't do this NG satisfies the condition for pass v
                      Grade > fail_threshold ~"OK",
                              T~"Sad")
         ) %>%
