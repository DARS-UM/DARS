...
d_sequence_transactions <- d_course %>%
  ...
  left_join(d_transcript, by = "Course ID") %>%
    ...
    #     T            ~ Grade),
    #   Grade = str_replace(string = Grade, pattern = ",",replace = "."),
    #   Grade = as.numeric(Grade)) %>% #this was already done in data preparation
...
##Rule induction
```{r}
#FURTURE: create filter by function
...
manual_confidence_rules <- pass_rules_df %>%
...
hi <- table(select(manual_confidence_rules,LHS,RHS))
#ggnetwork
...
We want to change the calculation of confidence and support. The new rules are:
confidence: number of transactions of RHS/ number of transactions of RHS and compliment= p(A^B)/P(BUBc)
support:number of transactions of right hand side/number of transactions of RHS and compliment= p(A^B)/p(AUBUBc)
lift: 
...
pass_rules_df
test <- pass_rules_df %>%
  separate(LHS, into=c("c1","c2", "c3", "c4", "c5", "c6","c7", "c8", "c9", "c10"), sep= ",", remove=F) #has values until c6

```

```{r}

length(unique(d_transcript$`Student ID`))
```
