...
pass_rules <- ruleInduction(frequent_pass_trajectories, confidence = 0.2, #we need to play with the parameters.
                    control    = list(verbose = TRUE))
...
```{r}
pass_rules_df %>%
  separate(rule, into=c("LHS", "RHS"), sep = "=>", remove = F) %>%
  filter(str_detect(RHS, "_Fail"))
```
