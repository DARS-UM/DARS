...
```{r helper function}
# helper function
join_d_course     <- function(df) df %>% left_join(select(d_course, - Period), by = c("Course ID"))

paste_ <- function(...) paste(..., sep = "_")
```

```{r d_course_informative}
...
```{r constants}
course_all  <- unique(d_transcript_augmented$`Course ID` )
student_all <- unique(d_transcript_augmented$`Student ID`)
...
d_support$TPH <- d_transcript_augmented %>%
...
```{r d_support G}
d_support$G <- d_transcript_augmented %>%
...
d_transactions$taken_PF_HL <- d_transcript_augmented %>%
...
d_transactions$G <- d_transactions$taken_PF_HL %>%
  ...
  mutate(row_of_origin = TRUE) %>%
  
  complete(
    nesting(sequenceID, eventID, item, Grade), grade_ceil,
    fill = list(row_of_origin = FALSE) # fill missing row_of_origin with FALSE
    ) %>%
...
save(d_transactions, file = "Output/transaction_df.RDATA")
```
...
```{r make_sequence}
make_sequence <- function(data, item){
  
  if(item == sym("item_TPF") | item == sym("item_THL")) return("Compute sequences that include courses nto taken yet by hand")
...
```{r transactions, cache = TRUE}
transactions <- tribble(
  ~ ID   ,  ~ data                   , ~ item,
  "taken", d_transactions$taken_PF_HL, sym("item"),
  "PF"   , d_transactions$taken_PF_HL, sym("item_PF"),
  "HL"   , d_transactions$taken_PF_HL, sym("item_HL"),
  "TPF"  , d_transactions$T_PF_HL    , sym("item_TPF"),
  "THL"  , d_transactions$T_PF_HL    , sym("item_TPF"),
  "G"    , d_transactions$G          , sym("item_g")
  ) %>%
  
  mutate(
    transactions = list(data, item) %>% pmap(make_transaction),
    sequences    = list(data, item) %>% pmap(make_sequence   )
    )

inspect(head(transactions$transactions[[3]], 10))
inspect(head(transactions$sequences   [[3]], 10))
```

```{r, include = FALSE}
rm(
  high_grade, pass_grade, course_all, student_all, n_students,
  d_transactions, d_support, d_transcript_cum,
  make_transaction, transactions, make_sequence, sequences
  )
```
