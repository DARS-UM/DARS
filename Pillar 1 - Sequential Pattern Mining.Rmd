...
clean_AR <- function(AR){
  ...
  AR %>%
    ...
    mutate(
      rules = str_remove(rules, pattern = "[{]"),
      rules = str_remove(rules, pattern = "[}]"),
      rules = str_remove(rules, pattern = "[{]"),
      rules = str_remove(rules, pattern = "[}]"), #this is needed 4 times, because it only removes the first instance
...
clean_rules <- function(rules){
  rules %>%
    ...
    # ) %>%
    mutate(
      rule = str_remove(rule, pattern = "[<]"),
      rule = str_remove(rule, pattern = "[{]"),
      rule = str_remove(rule, pattern = "[}]"),
      rule = str_remove(rule, pattern = "[>]"),
      rule = str_remove(rule, pattern = "[<]"),
      rule = str_remove(rule, pattern = "[{]"),
      rule = str_remove(rule, pattern = "[}]"),  #this is repeated, because it only removes the first instance
     # rule = str_remove(rule, pattern = "[>]"), #this is taken out because the rules is marked with a => so it removes that one. 
      `rhs.support` = confidence / lift
    ) %>%
    separate(rule, into = c("lhs", "rhs"), sep="=>") %>%
    mutate(
      lhs = str_remove(rule, pattern = "[<]"),
      lhs = str_remove(rule, pattern = "[{]"),
      lhs = str_remove(rule, pattern = "[}]"),
      lhs = str_remove(rule, pattern = "[>]"),
      rhs = str_remove(rule, pattern = "[<]"),
      rhs = str_remove(rule, pattern = "[{]"),
      rhs = str_remove(rule, pattern = "[}]"),
      rhs = str_remove(rule, pattern = "[>]")
    ) %>%
