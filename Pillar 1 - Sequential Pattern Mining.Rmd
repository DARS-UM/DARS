...
```{r helper function}
# helper function
join_d_course     <- function(df) df %>% left_join(d_course    , by = c("Course ID"))
join_d_transcript <- function(df) df %>% left_join(d_transcript, by = c("Course ID"))
...
# Year level
statistics$year <- provide_statistics(Year_numerical)
