...
clean_SR <- function(rules){
  ...
  rules %>%
    ...
    separate(
      lhs,
      into = c("LHS Course ID", "LHS Quality"),
      sep = "_",
      remove= FALSE
      ) %>%
    separate(
      rhs, 
      into = c("RHS Course ID", "RHS Quality"),
      sep = "_", 
      remove = FALSE
    )
}
...
SR_TPF <- d_transcript_cum                         %>%
  ...
  left_join(
    ...
    by= c("sequenceID" = "sequenceID", "eventID" = "eventID", "item" = "course_so_far")
    )                                              %>%
