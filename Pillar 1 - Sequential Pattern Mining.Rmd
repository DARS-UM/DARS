...
transactions_TNT <- make_transaction(data = d_transactions_not_taken, item = item_TNT)
transactions_TPF <- make_transaction(data = d_transactions_not_taken, item = item_TPF)
transactions_G   <- make_transaction(data = d_transactions_grade    , item = item_G  )
...
sequences_G   <- make_sequence(data = d_transactions_grade    , item = item_G  )
...
AR_G <- transactions_G %>%
  ...
  # include lhs.support by hand
  separate(
    lhs, 
    into = c("lhs_course", "lhs_grade_round"), 
    sep = "_",
    remove = FALSE,
    convert = TRUE
    ) %>%
  left_join(
    d_support_grade,
    by = c("lhs_course" = "item", "lhs_grade_round" = "grade_round")
    ) %>%
  mutate(
    lhs.support = support.grade
    ) %>%
  select(
    - support.grade
    ) %>%
  ...
  separate(
    rhs, 
    into = c("rhs_course", "rhs_grade_round"), 
    sep = "_",
    remove = FALSE,
    convert = TRUE
    ) %>%
  left_join(
    d_support_grade,
    by = c("rhs_course" = "item", "rhs_grade_round" = "grade_round")
    ) %>%
  mutate(
...
save(AR_taken, AR_PF, AR_HL, AR_TNT, AR_TPF, AR_G,
     file = "App/AR.RDATA")

# Remove objects
rm(clean_AR, my_apriori,
   transactions, transactions_PF, transactions_HL, transactions_G,
   AR_taken, AR_PF, AR_HL, AR_TNT, AR_G)
```
